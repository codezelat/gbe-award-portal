...
- The owner-approved 2026 LKR final offer is defined in `src/lib/domain/nomination-pricing.ts`: 16 September 2026, 12:00 PM to 17 September 2026, 12:00 PM Asia/Colombo, then LKR 85,000 for new nominations. Use this resolver at public display, initiation and final submission. Drafts do not lock the offer; final submission checks the acknowledged amount and returns `409 PRICE_CHANGED` without losing files. Preserve submitted payment snapshots and active attempts. Do not restart the absolute window on deployment, add a cron, or rewrite the stored cycle base fee to implement this schedule.
