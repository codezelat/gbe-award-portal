...
function Nomination({ row }: { row: InProgressRow }) {
  return (
    <div className="min-w-0">
      <p className="truncate text-xs font-medium text-foreground">{row.category ?? "Category not selected"}</p>
      {row.nomination ? (
        <Tooltip>
          <TooltipTrigger render={<button type="button" className="mt-1 block w-full truncate rounded-sm text-left text-sm leading-6 text-muted-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring" />}>
            {row.nomination}
          </TooltipTrigger>
        ...
        <Table className="table-fixed">
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-32 px-4"><span className="sr-only">Actions</span></TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {rows.map((row) => (
              <TableRow key={`${row.source}:${row.id}`}>
                ...
                <TableCell className="px-4 py-4">
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">{row.updatedLabel}</p>
                </TableCell>
                <TableCell className="px-4 py-4">
                  <div className="flex items-center justify-end gap-1">
                    <Button variant="ghost" className="h-11" render={<Link href={`/admin/in-progress/${row.id}?source=${row.source}`} aria-label={`View ${row.nomineeName}`} />}>View</Button>
                    {row.canDelete ? <DeleteDraftButton id={row.id} source={row.source} name={row.nomineeName} /> : null}
                  </div>
                </TableCell>
              </TableRow>
            ))}
      ...
      <div className="divide-y xl:hidden">
        {rows.map((row) => (
          <article key={`${row.source}:${row.id}`} className="min-w-0 p-4">
            ...
            <p className="mt-3 text-xs font-medium [overflow-wrap:anywhere]">{row.category ?? "Category not selected"}</p>
            {row.nomination ? <p className="mt-1 line-clamp-2 text-sm leading-6 text-muted-foreground [overflow-wrap:anywhere]">{row.nomination}</p> : null}
            <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-2">
              <Badge variant="secondary">{row.stepLabel}</Badge>
              <span className="text-xs text-muted-foreground">{row.updatedLabel}</span>
            </div>
            <div className="mt-3 flex items-center justify-between gap-3">
              <Button variant="outline" className="h-11" render={<Link href={`/admin/in-progress/${row.id}?source=${row.source}`} aria-label={`View ${row.nomineeName}`} />}>View details</Button>
              {row.canDelete ? <DeleteDraftButton id={row.id} source={row.source} name={row.nomineeName} /> : null}
            </div>
          </article>
        ))}
